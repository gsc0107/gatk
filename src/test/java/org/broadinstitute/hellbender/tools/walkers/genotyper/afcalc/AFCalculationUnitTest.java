...
public final class AFCalculationUnitTest extends BaseTest {
        ...
        for ( final int nSamples : Arrays.asList(1, 2, 3, 4) ) {
            ...
            for ( final double[] priors : Arrays.asList(flatPriors, humanPriors) ) { // , humanPriors) ) {
                for ( final AFCalculator model : calcs ) {
                    ...
                    // tri-allelic
                    if ((!priorName.equals("human")) && !(model instanceof OriginalDiploidExactAFCalculator)) // || model != generalCalc ) )
    ...
    private static final class NonInformativeData {
...
//                    Assert.assertEquals(actual.isPolymorphic(a, 0.0), expected.isPolymorphic(a, 0.0), "isPolymorphic with thread 0.0 for allele " + a);
    ...
    private static final class PNonRefData {
        final Genotype g;
    ...
    @Test(enabled = true, dataProvider = "PNonRefBiallelicSystematic")
...
//                    for ( final int nSamples : Arrays.asList(1000) ) {
    ...
    @Test(enabled = true, dataProvider = "polyTestProvider")
    ...
    @Test(enabled = true, dataProvider = "polyTestProviderLotsOfAlleles")
