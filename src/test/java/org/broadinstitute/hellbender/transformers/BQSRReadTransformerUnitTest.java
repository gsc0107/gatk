...
public final class BQSRReadTransformerUnitTest extends BaseTest{
    ...
    public void basicHierarchicalBayesianQualityEstimateTest() {
        ...
        for( double epsilon = 15.0; epsilon <= 60.0; epsilon += 2.0 ) {
            final double RG_Q = 45.0;
            final RecalDatum RG = makeRecalDatum(100000000L, RG_Q);
            final double Q = 30.0;
            final RecalDatum QS = makeRecalDatum(100000000L, Q);
            final RecalDatum COV = new RecalDatum(15L, 1.0, (byte)45.0); // no data here so Bayesian prior has a huge effect on the empirical quality
        ...
        for( double epsilon = 15.0; epsilon <= 60.0; epsilon += 2.0 ) {
            final double RG_Q = 45.0;
            final RecalDatum RG = makeRecalDatum(10L, RG_Q);
            final double Q = 30.0;
            final RecalDatum QS = makeRecalDatum(10L,Q);
            final RecalDatum COV = new RecalDatum(15L, 1.0, (byte)45.0); // no data here so Bayesian prior has a huge effect on the empirical quality
    ...
    public void repeatedAndUnorderedFixedQualities() {
        ...
        final List<Integer> quantizedQualsOrdered = Arrays.asList(11, 19);
        final List<Integer> quantizedQualsUnordered = Arrays.asList(19, 11, 19, 19);

        // Unordered and Ordered qmapping should be identical
        final byte[] qmappingUnordered = BQSRReadTransformer.constructStaticQuantizedMapping(quantizedQualsUnordered, true);
        final byte[] qmappingOrdered = BQSRReadTransformer.constructStaticQuantizedMapping(quantizedQualsOrdered, true);
    ...
    public void nearestVsRoundDown() {
        final List<Integer> fixedQuantizedQuals = Arrays.asList(10, 20, 30);

        final byte[] qmappingRoundDown = BQSRReadTransformer.constructStaticQuantizedMapping(fixedQuantizedQuals, true);
        final byte[] qmappingRoundNearest = BQSRReadTransformer.constructStaticQuantizedMapping(fixedQuantizedQuals, false);
    ...
    public void onlyOneFixedQualUsed() {
        ...
        final int singleQual = 10;
        final List<Integer> fixedQuantizedQuals = Arrays.asList(singleQual);

        final byte[] qmapping = BQSRReadTransformer.constructStaticQuantizedMapping(fixedQuantizedQuals, true);
