...
public final class ArtificialReadUtilsUnitTest extends BaseTest {
    ...
    public void basicReadIteratorTest() {
        final Iterator<GATKRead> iter = ArtificialReadUtils.mappedReadIterator(1, 100, 100);
        int count = 0;
        while (iter.hasNext()) {
            final GATKRead rec = iter.next();
            count++;
        }
    ...
    public void tenPerChromosome() {
        final ArtificialReadQueryIterator iter = ArtificialReadUtils.mappedReadIterator(1, 100, 10);
        int count = 0;
        while (iter.hasNext()) {
            final GATKRead rec = iter.next();
    ...
    public void onePerChromosome() {
        final ArtificialReadQueryIterator iter = ArtificialReadUtils.mappedReadIterator(1, 100, 1);
        int count = 0;
        while (iter.hasNext()) {
            final GATKRead rec = iter.next();
    ...
    public void basicUnmappedIteratorTest() {
        final Iterator<GATKRead> iter = ArtificialReadUtils.mappedAndUnmappedReadIterator(1, 100, 100, 1000);
        int count = 0;
