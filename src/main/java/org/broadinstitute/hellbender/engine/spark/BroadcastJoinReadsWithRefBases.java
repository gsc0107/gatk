...
public class BroadcastJoinReadsWithRefBases {
    ...
    public static JavaPairRDD<GATKRead, ReferenceBases> addBases(final ReferenceMultiSource referenceDataflowSource,
        ...
        final Broadcast<ReferenceMultiSource> bReferenceSource = ctx.broadcast(referenceDataflowSource);
        return reads.mapToPair(read -> {
            final SimpleInterval interval = bReferenceSource.getValue().getReferenceWindowFunction().apply(read);
    ...
    public static <T> JavaPairRDD<GATKRead, Tuple2<T, ReferenceBases>> addBases(final ReferenceMultiSource referenceDataflowSource,
        ...
        final JavaSparkContext ctx = new JavaSparkContext(keyedByRead.context());
        final Broadcast<ReferenceMultiSource> bReferenceSource = ctx.broadcast(referenceDataflowSource);
        return keyedByRead.mapToPair(pair -> {
            final SimpleInterval interval = bReferenceSource.getValue().getReferenceWindowFunction().apply(pair._1());
